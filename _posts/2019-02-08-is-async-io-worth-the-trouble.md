...
Nodejs has spread asynchronous I/O as way to write server side web applications.
A few years ago Nodejs popularized asynchronous I/O to write server side web applications.
Since then I often hear that web applications using an asynchronous server runtime
are faster and more scalable. And that is true some times, but I don't think that is the whole story and I want to take a closer look at the tradeoff involved in using async programming for server side web development, its benefits and its cost, and if async I/O is always a good idea.

In this article I'm assuming you're familiar with how asynchronous and synchronous I/O
work. If you're not, you can read the standard [node documentation about the subject](https://nodejs.org/en/docs/guides/blocking-vs-non-blocking/). It includes the standard rationale about why async I/O should be faster.
...
I'm reading 1MB from `/dev/urandom` to simulate some kind of I/O during a request, like accessing a database. 1MB may look too much, but that will emphasize the effects of I/O and given the [recent website obesity crisis](https://www.youtube.com/watch?v=iYpl0QVCr6U) it's actually a fairly standard weight for a web page these days.
...
Callbacks add so much complexity that we even have a name for too many of them: [callback hell](http://callbackhell.com/).
...
The multi thread mode is similar, but instead of a pool of processes, you have a pool of threads. A thread requires less memory than a process, but it also gives you less guarantees in terms of isolation, so there's a also tradeoff between simplicity and efficiency here.
...
This example is similar to the async one, but instead of only one process I've created 10.

Clustering a good practice even if you are only using async APIs. Nodejs is a single threaded environment, so in order to take full advantage of all the cores in your server you need to use some kind of clustering. That's why the cluster module is in the Nodejs standard library.
...
This might be true for multiprocess servers, but memory is cheap and it’s rarely the main bottleneck. I’d argue that [your brain is more likely to be the bottleneck](https://afcapel.com/articles/2019/01/18/cognitive-load-in-programming.html), so anything that you do decrease complexity will be more valuable that saving RAM.
...
and they only interact with the server sporadically. It seems a waste of resources to keep a process or thread blocked all the time waiting for the odd interaction. That's true, but in those cases you can [offload the connection management and message parsing to an application
...
`async` and `await` are wrappers on top of the `Promise` interface, and that's
...
in the first place**. In a way, we have implemented sync semantics (and performance) on top of an async API. But as we saw, the performance different was not really significant to begin with. After all this long journey, wouldn't have been easier have kept synchronous programming from the beginning?
